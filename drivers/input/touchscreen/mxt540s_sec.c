			     ...
			     const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%d", &data) != 1)
		dev_err(dev, "[TSP_KEY] keyled write error\n");

	if (1 == data)
		mxt->pdata->led_power_on(1);
	else if (2 == data)
		mxt->pdata->led_power_on(0);
	else
		dev_info(dev, "[TSP_KEY] %s wrong cmd %x\n",
			__func__, data);

#ifdef TSP_DEBUG_INFO
	dev_info(dev, "[TSP_KEY] Called value by HAL = %d\n", data);
#endif
...
#ifdef USE_TOUCH_KEYARRAY_T15
	fac_dev_ts = device_create(sec_class, NULL, 0, NULL, "sec_touchkey");
	ret = sysfs_create_group(&fac_dev_ts->kobj, &touchkey_attr_group);
	if (ret) {
		dev_err(&client->dev, "Failed to create touchkey sysfs group\n");
		goto free_mem;
	}
#endif
