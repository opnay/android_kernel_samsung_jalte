/* linux/drivers/video/backlight/s6e8fa0_G_mipi_lcd.c
 *
...
static int s6e8fa0_read_hover(struct lcd_info *lcd)
{
	int ret;
	u8 buf[3] = {0,};

	ret = s6e8fa0_read(lcd, LDI_HOVER_REG, buf, 3);
...
static int init_elvss_table(struct lcd_info *lcd, u8 *elvss_data)
{
	...
	for (j = 0; j < 2; j++) {
		...
		for (i = 0; i < ELVSS_STATUS_MAX; i++) {
			...
			lcd->elvss_table[j][i][1] = 0x2C;
			lcd->elvss_table[j][i][2] = elvss_data[1];
			lcd->elvss_table[j][i][2] += ELVSS_TABLE[i];
		}
...
struct mipi_dsim_lcd_driver s6e8fa0_G_mipi_lcd_driver = {
	.probe		= s6e8fa0_probe,
...
static int s6e8fa0_init(void)
{
	return 0;
#if 0
	s5p_mipi_dsi_register_lcd_driver(&s6e8fa0_G_mipi_lcd_driver);
