...
static unsigned char 	spi_buf[(PAKCET_SIZE * LOOP_RX_PAKET) + 5] = { 0 };
...
static int smt113j_spi_cmd_bufstat ( unsigned char *status )
{
	...
	spi_command.sts  = 0x00;
	
	/*** transfer data set ***/
	t[0].tx_buf = &spi_command;
	t[0].rx_buf = &spi_command;
	t[0].len    = sizeof ( spi_command );
...
static int smt113j_spi_cmd_pktread ( unsigned char *temp_buf )
{
	int 				ret = 0;
	int 				loop = 0;
	ioctl_spi_pktread 	spi_command = { 0 };
	...
	for ( loop = 0; loop < MAX_LOOP_CNT; loop++ )
	{
		...
		memcpy ( spi_buf, &spi_command, sizeof ( char ) * 5 );
			
		/*** transfer data set ***/
		t[0].tx_buf = spi_buf;
		t[0].rx_buf = spi_buf;
		t[0].len    = sizeof ( spi_buf ) + 3;
		...
		memcpy ( temp_buf, 
				 &spi_buf[5], 
				 sizeof ( char ) * ( LOOP_RX_PACKET_BYTE ));
								...
								loff_t * f_pos)
{
	...
	if ((MAX_BUFFER_PKT * PAKCET_SIZE) == pread)
	{
		pread = 0;
	}
...
static int smt113j_spi_probe(struct spi_device *spi)
{
	...
	spi->bits_per_word = 8;
	spi->mode = SPI_MODE_0;
	spi->max_speed_hz = (30 * 1000 * 1000); /* 30MHz */
